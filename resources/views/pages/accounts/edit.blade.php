...
<div class="container mx-auto px-4 py-6 md:py-12 max-w-4xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 font-kanit tracking-tight">แก้ไขรหัสบัญชี</h1>
            <p class="text-xs md:text-sm text-gray-500 font-kanit">แก้ไขข้อมูลผังบัญชี: <span class="text-emerald-600 font-bold">{{ $account->code }}</span></p>
        </div>
    ...
    <form id="accountForm" action="{{ route('accounts.update', $account->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-8">
                ...
                <div class="md:col-span-8 space-y-2">
                    ...
                    <select name="category" required
                        ...
                        <option value="asset" {{ old('category', $account->category) == 'asset' ? 'selected' : '' }}>1. สินทรัพย์ (Asset)</option>
                        <option value="liability" {{ old('category', $account->category) == 'liability' ? 'selected' : '' }}>2. หนี้สิน (Liability)</option>
                        <option value="equity" {{ old('category', $account->category) == 'equity' ? 'selected' : '' }}>3. ส่วนของเจ้าของ (Equity)</option>
                        <option value="revenue" {{ old('category', $account->category) == 'revenue' ? 'selected' : '' }}>4. รายได้ (Revenue)</option>
                        <option value="expense" {{ old('category', $account->category) == 'expense' ? 'selected' : '' }}>5. ค่าใช้จ่าย (Expense)</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-12 space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" name="name_th" value="{{ old('name_th', $account->name_th) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center space-x-4 h-[50px]">
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="is_group" value="1" class="w-4 h-4 text-emerald-600 border-gray-300 focus:ring-emerald-500" {{ old('is_group', $account->is_group) == '1' ? 'checked' : '' }}>
                            <span class="ml-2 text-sm font-kanit text-gray-600 group-hover:text-emerald-600 transition-colors">บัญชีคุม</span>
                        </label>
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="is_group" value="0" class="w-4 h-4 text-emerald-600 border-gray-300 focus:ring-emerald-500" {{ old('is_group', $account->is_group) == '0' ? 'checked' : '' }}>
        ...
        <div class="bg-gray-50/50 px-6 py-6 md:px-12 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center text-gray-400">
                <i class="fas fa-history text-emerald-500 mr-2 text-sm"></i>
                <span class="text-[11px] md:text-xs font-medium font-kanit leading-tight">แก้ไขข้อมูลล่าสุดเมื่อ: {{ $account->updated_at->format('d/m/Y H:i') }}</span>
            </div>
