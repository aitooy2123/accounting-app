<x-app-layout>
  ...
  @if ($errors->any())
    <div class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-xl">
      <div class="flex">
        ...
        <div>
          <h3 class="text-sm font-bold text-red-800">พบข้อผิดพลาด:</h3>
  ...
  <form action="{{ route('purchases.update', $purchase->id) }}" method="POST" id="purchaseForm">
    ...
    {{-- Hidden Inputs สำหรับส่งค่าคำนวณเงิน --}}
    <input type="hidden" name="subtotal" id="input-subtotal" value="{{ old('subtotal', $purchase->subtotal) }}">
    <input type="hidden" name="vat" id="input-vat" value="{{ old('vat', $purchase->vat) }}">
    <input type="hidden" name="total" id="input-total" value="{{ old('total', $purchase->total) }}">

    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 space-y-4 md:space-y-0">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 font-kanit">แก้ไขใบสั่งซื้อ: {{ $purchase->doc_no }}</h1>
        <p class="text-sm text-gray-500 font-kanit">ปรับปรุงข้อมูลรายการสั่งซื้อและสถานะการชำระเงิน</p>
      </div>
      <div class="flex space-x-3">
        <a href="{{ route('purchases.index') }}" class="px-5 py-2.5 bg-white border border-gray-200 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-50 transition-all">กลับหน้าหลัก</a>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
              <label class="block text-[11px] font-bold text-gray-400 uppercase mb-2 tracking-wider">ผู้จำหน่าย / เจ้าหนี้</label>
              <select name="supplier_id" class="w-full rounded-xl border-gray-200 text-sm py-2.5">
...
<div>
  <label class="block text-[11px] font-bold text-gray-400 uppercase mb-2">รับเข้าสาขา</label>
  <select name="branch_id" class="w-full rounded-xl border-gray-200 text-sm py-2.5">
    ...
    @endforeach
            ...
            <div>
              <label class="block text-[11px] font-bold text-gray-400 uppercase mb-2 tracking-wider">เลขที่เอกสาร</label>
              <input type="text" name="doc_no" value="{{ old('doc_no', $purchase->doc_no) }}" readonly
                     class="w-full rounded-xl border-gray-200 bg-gray-50 text-sm py-2.5 font-bold text-blue-600 cursor-not-allowed">
            </div>

            <div>
              <label class="block text-[11px] font-bold text-gray-400 uppercase mb-2 tracking-wider">สถานะเอกสาร</label>
              <select name="status" class="w-full rounded-xl border-gray-200 text-sm py-2.5 font-bold">
                <option value="ค้างชำระ" {{ old('status', $purchase->status) == 'ค้างชำระ' ? 'selected' : '' }} class="text-orange-500">ค้างชำระ</option>
                <option value="ชำระแล้ว" {{ old('status', $purchase->status) == 'ชำระแล้ว' ? 'selected' : '' }} class="text-emerald-500">ชำระแล้ว</option>
                <option value="ยกเลิก" {{ old('status', $purchase->status) == 'ยกเลิก' ? 'selected' : '' }} class="text-red-500">ยกเลิก</option>
              </select>
            </div>

            <div class="md:col-span-2">
              <label class="block text-[11px] font-bold text-gray-400 uppercase mb-2 tracking-wider">หมายเหตุ (Note)</label>
              <textarea name="note" rows="2" class="w-full rounded-xl border-gray-200 text-sm">{{ old('note', $purchase->note) }}</textarea>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div class="flex items-center justify-between mb-4 text-blue-600 font-bold text-lg font-kanit">
            ...
            <button type="button" onclick="addRow()" class="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100 font-bold">
          ...
          <div class="overflow-x-auto">
            <table class="w-full border-separate border-spacing-y-2" id="itemsTable">
              ...
              <tbody id="item-tbody">
                @php
                  // ดึงข้อมูลเดิมจาก Relation 'items' หากไม่มีให้ใช้ค่าเก่าจาก old()
                  $oldItems = old('items', $purchase->items->toArray());
                @endphp
                ...
                <tr class="bg-white border border-gray-100 rounded-lg shadow-sm item-row">
                  ...
                  <td class="py-3 px-4">
                    <input type="number" step="0.01" name="items[{{ $index }}][price]" value="{{ $item['price'] ?? 0 }}" oninput="calculateTotal()" class="price-input w-full border-none focus:ring-0 text-sm text-right p-0 font-bold text-blue-600">
                  </td>
      ...
      <div class="space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 font-kanit">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input type="date" name="doc_date" value="{{ old('doc_date', $purchase->doc_date->format('Y-m-d')) }}" class="w-full rounded-xl border-gray-200 text-sm py-2">
            </div>
            <div>
              <label class="block text-[11px] font-bold text-gray-400 uppercase mb-1">วันที่ครบกำหนด</label>
              <input type="date" name="due_date" value="{{ old('due_date', $purchase->due_date->format('Y-m-d')) }}" class="w-full rounded-xl border-gray-200 text-sm py-2">
            </div>
          </div>
        </div>
          ...
          <div class="flex flex-col space-y-2">
            @foreach([0, 7, 10] as $v)
              <label class="inline-flex items-center cursor-pointer">
                <input type="radio" name="vat_rate" value="{{ $v }}" onchange="calculateTotal()" class="text-blue-600" {{ old('vat_rate', (int)$purchase->vat_rate) == $v ? 'checked' : '' }}>
        ...
        <div class="bg-blue-600 p-6 rounded-3xl shadow-xl text-white relative overflow-hidden font-kanit">
          <div class="absolute -right-4 -top-4 opacity-10 text-8xl"><i class="fas fa-calculator"></i></div>
  ...
  <script>
    ...
    function addRow() {
      ...
      newRow.innerHTML = `
        <td class="py-3 px-4"><input type="text" name="items[${rowCount}][desc]" class="w-full border-none focus:ring-0 text-sm p-0 font-medium" placeholder="รายละเอียด..."></td>
        ...
        <td class="py-3 px-4"><input type="number" step="0.01" name="items[${rowCount}][price]" value="0.00" oninput="calculateTotal()" class="price-input w-full border-none focus:ring-0 text-sm text-right p-0 font-bold text-blue-600"></td>
        <td class="py-3 px-4 text-right text-sm font-bold text-gray-700 row-total">0.00</td>
    ...
    // รันการคำนวณครั้งแรกเมื่อโหลดหน้า
