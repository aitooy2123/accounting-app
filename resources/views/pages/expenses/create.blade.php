<x-app-layout>
  ...
  <form action="{{ route('expenses.store') }}" method="POST" id="expenseForm">
    ...
    {{-- Hidden field สำหรับส่ง amount (Subtotal) --}}
...
<input type="hidden" name="total" id="hidden_total" value="0">
  <script>
    ...
    function calculateTotal() {
      ...
      // อัปเดต hidden amount (ส่งไปยัง controller)
      document.getElementById('hidden_amount').value = subtotal.toFixed(2);
  ...
  const vat = subtotal * vatRate;
  const grandTotal = subtotal + vat; // ยอดสุทธิ

  // 2. อัปเดตยอดสุทธิลงใน Hidden Input เพื่อส่งไปบันทึก (ฟิลด์ total)
  document.getElementById('hidden_total').value = grandTotal.toFixed(2);
