<x-app-layout>
  ...
  <form action="{{ route('sales.update', $sale->id) }}" method="POST" id="salesForm">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 space-y-4 md:space-y-0">
      <div>
        ...
        <p class="text-sm text-gray-500 font-kanit">แก้ไขเอกสารการขาย #{{ $sale->invoice_no ?? $sale->id }}</p>
      </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" id="company_name" value="{{ old('company_name', $sale->company_name ?? '') }}" readonly
            ...
            <div>
              ...
              <input type="text" name="tax_id" id="tax_id" value="{{ old('tax_id', $sale->tax_id) }}" readonly class="w-full rounded-xl border-gray-200 bg-gray-50 text-sm py-2.5 text-gray-500">
              @error('tax_id')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div class="md:col-span-2">
              ...
              <input type="text" name="address" id="address" value="{{ old('address', $sale->address) }}" readonly class="w-full rounded-xl border-gray-200 bg-gray-50 text-sm py-2.5 text-gray-500">
              @error('address')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
              @enderror
            </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div class="overflow-x-auto">
            <table class="w-full border-separate border-spacing-y-2" id="itemsTable">
              ...
              <tbody id="item-tbody">
                @php
                  // ดึงรายการสินค้าเดิมจาก model หรือ old input
                  $oldItems = old('items');
                  if (is_null($oldItems) && $sale->items) {
                      $oldItems = $sale->items->map(function($item) {
                          return [
      ...
      <div class="space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 font-kanit">
          ...
          <div class="space-y-4">
           ...
           <input type="date" name="doc_date" value="{{ old('doc_date', $sale->doc_date ? \Carbon\Carbon::parse($sale->doc_date)->format('Y-m-d') : date('Y-m-d')) }}" class="w-full rounded-xl border-gray-200 text-sm py-2 @error('doc_date') border-red-500 @enderror">
            </div>
            <div>
  ...
  <script>
    ...
    // อัปเดตข้อมูลลูกค้า (เฉพาะ tax, address, company) ไม่ยุ่งกับสาขา (branch จะคงค่าจากฟอร์ม)
    function populateCustomerFields(select) {
    ...
    // เวลาเปลี่ยนลูกค้า: อัปเดต tax, address, company และเซ็ตสาขาจาก branch_id ของลูกค้า (ถ้ามี)
    function updateCustomerInfo(select) {
      populateCustomerFields(select);
      const branchId = select.options[select.selectedIndex].getAttribute('data-branch-id');
    ...
    window.onload = function() {
      // ถ้ามี customer ถูกเลือก (จาก old หรือ sale) ให้ดึงข้อมูล company/tax/address โดยไม่เปลี่ยน branch
      const customerSelect = document.querySelector('select[name="customer_id"]');
      if (customerSelect && customerSelect.value) {
        populateCustomerFields(customerSelect);
      }
      // คำนวณยอดรวม
      calculateTotal();

      // ป้องกัน branch_select ถูก override จาก customer (เผื่อกรณีแก้ไข)
      // ค่า branch_select จะถูกเซ็ตจาก old หรือ sale อยู่แล้วใน HTML
    };
  </script>
</x-app-layout>
