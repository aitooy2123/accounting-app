...
<div class="container mx-auto px-4 py-6 md:py-12 max-w-4xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 font-kanit tracking-tight">แก้ไขข้อมูลบริษัท</h1>
            <p class="text-xs md:text-sm text-gray-500 font-kanit">
                แก้ไขรายละเอียดของ <span class="text-blue-600 font-bold">{{ $company->name }}</span>
            </p>
        </div>
    ...
    <form id="companyForm"
        ...
        @csrf
        @method('PUT') {{-- สำคัญมากสำหรับการ Edit --}}
        ...
        <div class="p-6 md:p-12 space-y-6 md:space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-8">
                <div class="md:col-span-4 space-y-2">
                    ...
                    <input type="text" value="{{ $company->code }}" readonly
                ...
                <div class="md:col-span-8 space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" name="tax_id" value="{{ old('tax_id', $company->tax_id) }}" maxlength="13"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="address" rows="3"
                              class="block w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all resize-none">{{ old('address', $company->address) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50/50 px-6 py-6 md:px-12 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center text-gray-400">
                ...
                <span class="text-[11px] md:text-xs font-medium font-kanit leading-tight">แก้ไขล่าสุดเมื่อ: {{ $company->updated_at->format('d/m/Y H:i') }}</span>
            </div>

            <label class="relative inline-flex items-center cursor-pointer group">
                <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $company->is_active ? 'checked' : '' }}>
                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"></div>
                <span class="ml-3 text-xs font-bold text-gray-600 font-kanit group-hover:text-green-600 transition-colors">สถานะปัจจุบัน</span>
            </label>
        </div>
    </form>
</div>
